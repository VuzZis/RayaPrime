...
import com.vuzz.storymoding.capability.CapabilityPM;
import com.vuzz.storymoding.capability.CapabilityPMProvider;
import com.vuzz.storymoding.capability.PM;
import com.vuzz.storymoding.effects.ModEffects;
import com.vuzz.storymoding.world.gen.ModOreGeneration;

@Mod.EventBusSubscriber(modid = StoryModing.MOD_ID)
public class ModEvents {

    public static final ResourceLocation PM_ID = new ResourceLocation(StoryModing.MOD_ID, "pm");
    ...
    public static void onPlayerWakeUpEvent(PlayerWakeUpEvent event) {
        if(!event.getEntity().getEntityWorld().isRemote) {
            ...
            if(player.getPersistentData().getBoolean("hasraya")) {
                player.sendMessage(new TranslationTextComponent("message."+StoryModing.MOD_ID+".wakeup"),Util.DUMMY_UUID);
            }
        }
    }
    ...
    public static void onPlayerDestroyItemEvent(PlayerDestroyItemEvent event) {
        if(!event.getEntity().getEntityWorld().isRemote) {
            PlayerEntity player = (PlayerEntity) event.getEntity();
            if(player.getPersistentData().getBoolean("hasraya")) {
                //player.sendMessage(new TranslationTextComponent("message."+StoryModing.MOD_ID+".itembroke"),Util.DUMMY_UUID);
            }
        }
    }
    ...
    public static void onLivingDeathEvent(LivingDeathEvent event) {
        ...
        if(!event.getEntity().getEntityWorld().isRemote) {
            ...
            if(killer != null && killer instanceof PlayerEntity) {
                ...
                String ablob = new TranslationTextComponent("title."+StoryModing.MOD_ID+".ablob").getString();
                player.sendStatusMessage(new StringTextComponent(ablob+pmToGive+" "+new TranslationTextComponent("title."+StoryModing.MOD_ID+".pm").getString()), true);
            }
        }
    }
}
