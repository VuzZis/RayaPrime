...
public class InactiveOculus extends Item implements Implant {
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int tick, boolean bool) {
        ...
        if(player.getFoodStats().getFoodLevel() > 1 && stack.getTag().getFloat("energy") < 29950) {
            if(ticks % 400 == 0) {
                stack.getTag().putFloat("energy",stack.getTag().getFloat("energy")+100f);
    ...
    public ActionResultType onItemUseFirst(ItemStack stack, ItemUseContext context) {
        ...
        else 
        {
            ...
            } else 
            {
                ...
                EntityType<OculusEntity> raya = ModEntityTypes.OCULUS.get();
                OculusEntity rayaEntity = (OculusEntity) raya.spawn((ServerWorld) context.getWorld(), stack, context.getPlayer(), context.getPlayer().getPosition(), 
                    SpawnReason.DISPENSER, false, false);
                    
                context.getPlayer().getPersistentData().putBoolean("hasoculus", true);
        ...
        text.add(
            new StringTextComponent(
                new TranslationTextComponent("tooltip.haloterra.name")
                    .getString()
                +"Occur D. Lust"+"\n"+new TranslationTextComponent("tooltip.haloterra.oculus").getString()
            )
        );
