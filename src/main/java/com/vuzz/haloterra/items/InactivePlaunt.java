...
public class InactivePlaunt extends Item {
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int tick, boolean bool) {
        ...
        if(player.getFoodStats().getFoodLevel() > 1 && stack.getTag().getFloat("energy") < 19950) {
            if(ticks % 400 == 0) {
                stack.getTag().putFloat("energy",stack.getTag().getFloat("energy")+120f);
    ...
    public ActionResultType onItemUseFirst(ItemStack stack, ItemUseContext context) {
        ...
        else 
        {
            ...
            } else 
            {
                ...
                EntityType<PlauntEntity> raya = ModEntityTypes.PLAUNT.get();
                PlauntEntity rayaEntity = (PlauntEntity) raya.spawn((ServerWorld) context.getWorld(), stack, context.getPlayer(), context.getPlayer().getPosition(), 
                    SpawnReason.DISPENSER, false, false);
                    
                context.getPlayer().getPersistentData().putBoolean("hasplaunt", true);
