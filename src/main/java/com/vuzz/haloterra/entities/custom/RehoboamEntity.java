...
public class RehoboamEntity extends ShoulderRidingEntity implements IFlyingAnimal {
    ...
    private static float energyConsumtion = 1f;
    private static float walkingConsumtion = 2f;
    ...
    private void returnAsImplant() {
        ...
        if(owner instanceof PlayerEntity) {
            ...
            ItemStack item = new ItemStack(ModItems.INACTIVE_REHOBOAM.get());
            if(player.canPickUpItem(item)) {
                item.setTag(nbt);
                player.addItemStackToInventory(item);
                player.getPersistentData().putBoolean("hasrehoboam", false);
                remove();
            } else {
                remove();
                item.setTag(nbt);
    ...
    public void tick() {
        ...
        if(!(owner instanceof PlayerEntity)) return;
            ...
            if(distanceBetween > STAY_DISTANCE) {
                ...
                setMotion(0, clamp(getOwner().getPosYEye()-getPosY()-1.7,-0.2,0.2), 0);
                getNavigator().clearPath();
            }
                List<Entity> entitiesClose = world.getEntitiesWithinAABBExcludingEntity(this,getBoundingBox().expand(10,10,10));
                for(int i = 0; i < entitiesClose.size(); i++) {
                    ...
                    if(entityToBeat.getClassification(true) == EntityClassification.MONSTER) {
                        lookAt(Type.EYES,new Vector3d(entityToBeat.getPosX(),entityToBeat.getPosY(),entityToBeat.getPosZ()));
                        entityToBeat.addPotionEffect(new EffectInstance(Effects.SLOWNESS,2,255-((entitiesClose.size()-1)*10)));
                        curEnergy-=10;
                    }
                }
